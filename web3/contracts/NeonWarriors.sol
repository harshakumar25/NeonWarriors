...
/// @title NeonWarriors
/// @notice This contract handles the token management and battle logic for the NeonWarriors game (v1.1.0)
/// @author NeonWarriors Team
...
contract NeonWarriors is ERC1155, Ownable, ERC1155Supply, ReentrancyGuard {
  ...
  uint256 public constant CYBER_REAPER = 0;
  uint256 public constant GRID_STALKER = 1;
  uint256 public constant NEON_PHOENIX = 2;
  uint256 public constant DATA_WRAITH = 3;
  uint256 public constant VOID_RUNNER = 4;
  uint256 public constant GLITCH_MONARCH = 5;
  ...
  struct Player {
    ...
    uint256 playerMana; /// @param playerMana player energy; affected by battle results
    uint256 playerHealth; /// @param playerHealth player integrity; affected by battle results
    bool inBattle; /// @param inBattle boolean to indicate if a player is in the grid
  }
  ...
  function _createRandomNum(uint256 _max, address _sender) internal view returns (uint256 randomValue) {
    uint256 randomNum = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp, _sender)));
  ...
  function attackOrDefendChoice(uint8 _choice, string memory _battleName) external nonReentrant {
    Battle memory _battle = getBattle(_battleName);
  ...
  function _isCriticalHit(address _sender) internal view returns (bool) {
    uint256 rand = uint256(keccak256(abi.encodePacked(block.timestamp, _sender))) % 100;
    return rand < 15; // 15% chance for a critical hit
  }
  ...
  function _resolveBattle(Battle memory _battle) internal {
    ...
    if (p1.move == 1 && p2.move == 1) {
      ...
      bool isCrit = _isCriticalHit(_battle.players[1]);
      uint256 damaged = isCrit ? p2.attack * 2 : p2.attack;
      ...
      if (damaged >= PHAD) {
        ...
        if(p1.defense > damaged) {
          healthAfterAttack = p1.health;
        } else {
          healthAfterAttack = PHAD - damaged;
